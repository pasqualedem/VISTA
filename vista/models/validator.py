...
Extends ``DetectionValidator`` with:
  ...
  - Structured output after each validation run:
      <save_dir>/metrics_summary.json   scalar + per-class metrics
      <save_dir>/metrics_summary.csv    same, tabular
...
class VISTAValidator(DetectionValidator):
    ...
    def __call__(self, trainer=None, model=None):
        ...
        self.finalize_metrics()   # calls our override below
        self.print_results()
    ...
    def finalize_metrics(self) -> None:
        ...
        try:
            ...
            LOGGER.warning(f"VISTAValidator: could not save structured outputs — {exc}")
    ...
    def _save_metrics_json(self) -> None:
        ...
        payload = {
            ...
            "speed_ms": {k: round(v, 3) for k, v in self.speed.items()},
            "per_class": [
